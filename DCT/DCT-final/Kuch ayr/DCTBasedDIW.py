# -*- coding: utf-8 -*-
"""
Created on Wed Oct  6 12:39:16 2021
...
# Reading watermark 
watermark = cv.imread('SVNIT_LOGO_final.jpg',0)
...
for i in range(0, np.shape(img_1)[0], 8): 
    for j in range(0, np.shape(img_1)[1], 8): 
        dct_of_img[i+6:i+8, j+6:j+8] = np.reshape((wm_1_flat[m:m+4]), (2,2)).astype(np.int64)
        m = m + 4 
...
embedded_image_dash = (idct_of_img + img_normalizer)*100 
...
for i in range(0, np.shape(img_1)[0], 8): 
    for j in range(0, np.shape(img_1)[1], 8): 
        temp_array = dct_of_img_again[i+6:i+8, j+6:j+8].astype(np.int64) 
        wm_extracted_flat[n:n+4] = np.ravel(temp_array).astype(np.int64) 
        n = n + 4 
# wm_extracted = np.zeros(np.shape(wm_1)) 

wm_extracted = np.reshape(wm_extracted_flat, (np.shape(wm_1))) 
wm_final = wm_extracted + wm_normalizer
